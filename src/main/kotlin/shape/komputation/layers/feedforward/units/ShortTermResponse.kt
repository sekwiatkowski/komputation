...
class ShortTermResponse(
    ...
    fun forward(step : Int, state : DoubleMatrix, input : DoubleMatrix, forget : DoubleMatrix): DoubleMatrix {

        val shortTermMemory = this.forgetting[step].forward(state, forget)

        val weightedShortTermMemory = this.memoryWeighting.forwardStep(step, shortTermMemory)
        ...
        val shortTermResponse = this.activations[step].forward(preActivation)
