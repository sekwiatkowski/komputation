...
class HighwayLayer(
    ...
    private val addition : AdditionCombination) : ContinuationLayer(name), Optimizable {
    ...
    override fun forward(input: DoubleMatrix): DoubleMatrix {
        ...
        // H(x)
        val transformed = this.transformation.forward(input)

        // T(x)
        val transformationFraction = this.transformationFraction.forward(input)

        // H(x) (.) T(x)
        val transformationComponent = this.transformationHadamard.forward(transformed, transformationFraction)

        // 1 - T(x)
        val carryFraction = this.counterProbability.forward(transformationFraction)
